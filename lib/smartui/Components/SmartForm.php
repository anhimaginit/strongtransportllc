...
class SmartForm extends UI
{
	...
	public static function print_field($name, $type = self::FORM_FIELD_INPUT, $properties = array(), $options = false, $return = false, $isshow=true)
	{
		if (isset($isshow) && ($isshow == true || $isshow == 'true')){
			...
			case self::FORM_FIELD_RADIO:
				...
				foreach ($items as $item) {
					...
					$item_html = self::_get_field_html($new_item_prop['name'], self::FORM_FIELD_INPUT, array(
						...
						'disabled' => $new_item_prop['disabled'],
						'id' => $new_item_prop['id']
					), true);
