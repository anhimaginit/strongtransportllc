...
var calc_navbar_height = function () {
	...
	initApp = (function (app) {
		...
		app.SmartActions = function () {
			...
			var smartActions = {
				// Deny permission to access file
				deniedPermission: function () {
					$.SmartMessageBox({
						title: "<label class='txt-color-orangeDark bold'>Refuse Permission</label>",
				...
				fileDontExists: function () {
					$.SmartMessageBox({
						title: "<label class='txt-color-orangeDark bold'>Link Don't Exist</label>",
						content: "The link you find don't exist. Go back",
						buttons: '[OK]'
			...
			$.root_.on('click', '[data-action=deniedPermission]', function (e) {
				smartActions.deniedPermission();
				e.preventDefault();
				$this = null;
				//clear memory reference
			});
...
if ($.navAsAjax || $(".google_maps")) {
	...
	window.loadGoogleMaps = function () {
		...
		script_tag.setAttribute("src", "https://maps.google.com/maps/api/js?key=AIzaSyARsIKHVjH0zP6DLu20pVsdQ0FnxUztX5Q&libraries=places&callback=gMapsCallback");
...
function checkURL() {
	...
	// var url = window.location.href.split('#').splice(1).join('#');
	...
	if (url) {
		...
		if (!url.includes('.php') && !url.includes('?') && !(['html', 'js', 'css', 'json', 'png', 'jpg', 'pdf', 'svg', 'doc', 'docx', 'xls', 'xlsx', 'gif', 'scv', 'mp3', 'mp4'].includes(url.split('.').pop()))) {
			url += '.php';
		} else if (!url.includes('.php') && url.includes('?')) {
			var tmp = url.split('?').shift();
			var tmp2 = url.substring(tmp.length);
			url = tmp.concat('.php', tmp2);
		} else if (url.includes('.php') && url.includes('?') && url.indexOf('.php') > url.indexOf('?')) {
			var tmp = url.split('?').shift();
			var tmp2 = url.substring(tmp.length);
			url = tmp.concat('.php', tmp2);
		}
		...
		//update hash
		window.location.hash = '#ajax/dashboard';
